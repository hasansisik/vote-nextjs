...
import { getUserCreatedTests, updateTest } from "@/redux/actions/userActions";
...
export default async function EditTestPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <EditTestPageClient testId={testId} />;
}

function EditTestPageClient({ testId }: { testId: string }) {
  const dispatch = useDispatch<AppDispatch>();
  const router = useRouter();
  const { userCreatedTests, testsLoading, user } = useSelector((state: any) => state.user);
  ...
  useEffect(() => {
    if (user?.role === 'admin' && userCreatedTests.length === 0) {
      dispatch(getUserCreatedTests());
    }
  }, [dispatch, user, userCreatedTests.length]);

  useEffect(() => {
    if (userCreatedTests.length > 0 && testId) {
      const foundTest = userCreatedTests.find((t: any) => t._id === testId);
      if (foundTest) {
        setTest(foundTest);
        setFormData({
          title: foundTest.title || "",
          description: foundTest.description || "",
          coverImage: foundTest.coverImage || "",
          headerText: foundTest.headerText || "",
          footerText: foundTest.footerText || "",
          category: foundTest.category || "",
          isActive: foundTest.isActive !== undefined ? foundTest.isActive : true,
        });
        setOptions(foundTest.options || [
          { title: "", image: "", customFields: [] },
          { title: "", image: "", customFields: [] },
        ]);
      }
    }
  }, [userCreatedTests, testId]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Update başarılı olduktan sonra test listesini yenile
      await dispatch(getUserCreatedTests());
  ...
  return (
    <div className="space-y-6 px-6">
      {/* WordPress-style action buttons at the top */}
      <div className="flex justify-between items-center bg-white border-b border-gray-200 pb-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold">Oylama Düzenle</h1>
            <p className="text-muted-foreground">
              Oylama bilgilerini güncelleyin
            </p>
          </div>
        </div>
        ...
        <div className="flex gap-3">
          ...
          >
            ...
            {testsLoading ? "Güncelleniyor..." : "Değişiklikleri Kaydet"}
          </Button>
        </div>
      </div>
