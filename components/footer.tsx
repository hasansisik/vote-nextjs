    ...
    <footer className="bg-gray-50 border-t border-gray-200 mt-auto">
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          ...
          {/* Kategoriler - İlk 5 */}
          <div>
            <h4 className="text-sm font-semibold text-gray-900 mb-3">{t('categories')}</h4>
            {menuLoading ? (
                ...
                {[...(allMenus || [])].sort((a: any, b: any) => (a.order || 0) - (b.order || 0)).slice(0, 5).map((item: any, index: number) => (
                  <li key={index}>
                    ...
                    >
                      ...
                      ></div>
{item.testCategory.name?.[locale] || item.testCategory.name?.tr || item.testCategory.name || t('category')}
                    </button>
                  </li>
                ))}
              </ul>
